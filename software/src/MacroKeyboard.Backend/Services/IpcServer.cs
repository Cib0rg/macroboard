...
public class IpcServer : IIpcServer, IDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // SO_REUSEADDR allows quick restart even when a previous connection's socket
            // is still in TIME_WAIT. Must be set before Start().
        ...
        {
            _logger.LogError(ex, "Failed to start IPC Server on port {Port} — is another instance running?", _port);
            throw;
        }
    }
