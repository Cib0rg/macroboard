...
///
/// Payload layout: [button_id(1)][flags(1)][text_len(1)][text(0..53)]
...
public class PluginDisplayCommand
{
    ...
    public async Task<bool> ExecuteAsync(byte buttonId, string text, bool isOn,
        CancellationToken ct = default)
    {
        var textBytes = Encoding.UTF8.GetBytes(text ?? string.Empty);
        // Max text = PayloadSize(56) - 3 header bytes = 53 bytes
        var maxText = ProtocolConstants.PayloadSize - 3;
        if (textBytes.Length > maxText)
            textBytes = textBytes[..maxText];

        var payload = new byte[3 + textBytes.Length];
        payload[0] = buttonId;
        payload[1] = (byte)(isOn ? 0x01 : 0x00);  // flags: bit 0 = isOn
        payload[2] = (byte)textBytes.Length;
        Array.Copy(textBytes, 0, payload, 3, textBytes.Length);
