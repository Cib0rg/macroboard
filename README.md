...
- ✅ **Конфигурация** — хранение настроек кнопок и энкодера в SPIFFS
...
- ✅ **Асинхронная запись SPIFFS** — `save_task` на Core 0, очередь 20 слотов; протокол не блокируется на I/O, sync fallback при нехватке слотов
- ✅ **Длинный текст клавиатуры** — тексты до 4096 байт хранятся в SPIFFS (пароли, авто-ответы, ключи)
...
- ✅ **Diff-based синхронизация профиля** — при Send пересылаются только изменённые кнопки; fingerprint учитывает image path+mtime, action type+JSON, LED; типовая правка одной кнопки: ~5 с вместо ~39 с
...
│       ├── storage/             # NVS, SPIFFS, профили, изображения, длинный текст, async save_task
...
| 0x38–0x3A | TEXT_TRANSFER | Передача длинного текста (start/chunk/end); для текстов > 44 байт |
...
### SPIFFS-хранилище длинного текста

Для Keyboard-действий с текстом длиннее 44 байт (пароли, авто-ответы, API-ключи) используется отдельный механизм передачи через команды 0x38–0x3A. На устройстве текст хранится в файле `/storage/txt_{profile_id}_{button_id}.bin` в SPIFFS-разделе (лимит 4096 байт). В `action_data` кнопки проставляется флаг `data[2]=0x01`, по которому прошивка загружает текст из файла при нажатии. Маршрутизация прозрачна: ПО автоматически выбирает между inline и SPIFFS в зависимости от длины текста.
