...
- ✅ **Асинхронная запись SPIFFS** — `save_task` на Core 0, очередь 20 слотов; протокол не блокируется на I/O, sync fallback при нехватке слотов; полная синхронизация ~17 изображений: ~11 с вместо ~39 с
- ✅ **SPIFFS хранилище текстов** — тексты до 4096 байт в SPIFFS по 5-компонентному ключу `(profile/folder/button/action_slot/step_index)`; покрывает: Keyboard > 44 байт, long press Keyboard, каждый шаг Sequence с длинным текстом, Sequence-блоб > 51 байт
