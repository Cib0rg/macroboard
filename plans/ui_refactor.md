...
- `📁` на кнопке — это папка; содержит вложенный набор кнопок
- Кнопки энкодера — отдельная компактная секция ниже сетки

### Навигация по папкам

Canvas показывает один уровень иерархии. Над сеткой — breadcrumb:

```
Root  >  Work Apps  >  Dev Tools
[←]  ────────────────────────────
...
- **Одиночный клик** на кнопку-папку — выбрать её; правая Config Panel показывает
  внешний вид папки на текущем уровне (имя, картинка, LED)
- **Двойной клик** на кнопку-папку (или кнопка «Open folder» в Config Panel) —
  провалиться внутрь; canvas обновляется до содержимого папки
- **Breadcrumb** — кликабельный, возврат на любой уровень вверх; `[←]` — на один уровень вверх
- Root-уровень: breadcrumb не показывается (не нужен)
...
- Новый `DeviceCanvasViewModel.cs`
...
- Папки — особый тип тайла с иконкой 📁; одиночный клик выбирает, двойной — проваливается

**Навигационный стек папок:**
- `DeviceCanvasViewModel` держит `Stack<FolderLevel>` где `FolderLevel` = `{ string Name, List<ButtonModel> Buttons }`
- `CurrentButtons` — верхушка стека; именно их рисует canvas
- `BreadcrumbPath` — `ObservableCollection<string>` для отображения пути; пустой на Root
- `NavigateInto(folderButton)` — пушит новый уровень в стек, обновляет `CurrentButtons`
- `NavigateUp()` — попает стек; `CanNavigateUp = Stack.Count > 1`
- Клик по элементу breadcrumb — попает стек до нужного уровня
